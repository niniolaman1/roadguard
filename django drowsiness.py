...
# Django setup
# -----------------------------
sys.path.insert(0, '/home/ninipi/Documents/capstone/roaguard')
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'roaguard.settings')
django.setup()

from django.utils import timezone
from monitoring.models import Trip, DrowsinessEvent
...
# Start a Trip
# -----------------------------
trip = Trip.objects.create(start_time=timezone.now())
print(f"Trip {trip.id} started at {trip.start_time}")
...
alert_saved = False  # prevents saving duplicate events per closure episode
...
try:
            ...
            if eye_closed_now:
                if not eyes_closed:
                    ...
                    if duration >= DROWSY_SECONDS:
                        ...
                        # Save event once per closure episode
                        if not alert_saved:
                            ...
                            print(f"Event saved: {severity} ({duration:.2f}s)")
                            alert_saved = True
            else:
                eyes_closed = False
                eyes_closed_start_time = None
    ...
    trip.save()
    print(f"Trip {trip.id} ended. Events recorded: {trip.events.count()}")
